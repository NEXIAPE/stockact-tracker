...
function SectionTable({ table }: { table: NonNullable<ProcedureSection["table"]> }) {
  return (
    <div className="overflow-x-auto">
      <table className="w-full border-collapse text-sm">
        <thead>
          <tr>
            {table.headers.map((h) => (
              <th key={h} className="border border-slate-200 bg-slate-50 px-3 py-2 text-left font-semibold text-slate-600">
                {h}
              </th>
            ))}
          </tr>
          ...
          {table.rows.map((row, ri) => (
            <tr key={ri} className="align-top">
              {row.map((cell, ci) => (
                <td
                  key={ci}
                  className={`border border-slate-200 px-3 py-2 text-slate-700 ${ci === 0 ? "whitespace-nowrap font-medium text-slate-500" : ""}`}
...
function SectionBody({ section }: { section: ProcedureSection }) {
  if (section.table) return <SectionTable table={section.table} />;
